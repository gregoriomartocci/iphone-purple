  ...
  return (
    <div className="shell py-12 sm:py-16">
      <header className="max-w-2xl">
        <h1 className="text-3xl font-semibold sm:text-5xl">Notas</h1>
        <p className="text-muted-foreground mt-4 text-lg leading-relaxed">
          Lo que aprendimos vendiendo y reparando equipos, contado sin vueltas.
        </p>
      </header>

      {posts.length === 0 ? (
        <p className="text-muted-foreground mt-16">Todavía no publicamos ninguna nota.</p>
      ) : (
        <>
          <Link href={`/blog/${lead.slug}`} className="group mt-14 block">
            <div className="grid gap-8 md:grid-cols-2 md:items-center">
              ...
              <div>
                ...
                <h2 className="text-ink mt-2 text-2xl leading-snug font-semibold sm:text-3xl">
                  {lead.title}
                </h2>
            ...
            <div className="border-line mt-20 grid gap-10 border-t pt-14 sm:grid-cols-2 lg:grid-cols-3">
              {rest.map((post) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group">
                  ...
                  <h2 className="text-ink mt-1.5 leading-snug font-medium">
                    {post.title}
                  </h2>
